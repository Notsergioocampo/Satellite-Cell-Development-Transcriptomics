...
setup_logging <- function(log_dir = "results/logs", log_level = "INFO") {
  ...
  basicConfig(level = log_level, file = logfile)
...
export_results <- function(results, output_dir = "results", format = c("RData", "csv", "json")) {
  ...
  for (result_name in names(results)) {
    ...
    # Export CSV format (for data frames)
...
annotate_genes <- function(gene_list, species = "Mus musculus") {
  ...
  tryCatch({
    if (species == "Mus musculus") {
      ...
      # Map gene symbols to various annotations
      annotations <- list(
        entrez_id = mapIds(org.Mm.eg.db, keys = gene_list, keytype = "SYMBOL", column = "ENTREZID"),
        gene_name = mapIds(org.Mm.eg.db, keys = gene_list, keytype = "SYMBOL", column = "GENENAME"),
        go_terms = mapIds(org.Mm.eg.db, keys = gene_list, keytype = "SYMBOL", column = "GO"),
        kegg_pathways = mapIds(org.Mm.eg.db, keys = gene_list, keytype = "SYMBOL", column = "PATH")
      )
    ...
    return(data.frame(gene_symbol = gene_list))
  })
}
...
save_plot_with_metadata <- function(plot, filename, width = 10, height = 8, dpi = 300, metadata = NULL) {
  # Save plot
...
generate_summary_report <- function(results_list, output_file = "results/pipeline_summary.html") {
...
create_html_report <- function(summary_stats, results_list) {
  ...
  for (step_name in names(results_list)) {
    step_result <- results_list[[step_name]]
...
handle_error <- function(error, context = "") {
  ...
  error_message <- paste0("[", error_timestamp, "] ERROR in ", context, ": ", conditionMessage(error))
  ...
  # Save error report
  error_file <- file.path("results", "logs", paste0("error_", format(error_timestamp, "%Y%m%d_%H%M%S"), ".json"))
...
check_dependencies <- function(required_packages) {
  ...
  if (length(missing_packages) > 0) {
    logwarn(paste("Missing packages:", paste(missing_packages, collapse = ", ")))
...
install_missing_packages <- function(packages) {
  if (length(packages) > 0) {
    loginfo(paste("Installing missing packages:", paste(packages, collapse = ", ")))
...
get_system_info <- function() {
  sys_info <- list(
    ...
    memory_limit = if (exists("memory.limit")) memory.limit() else "Not available",
