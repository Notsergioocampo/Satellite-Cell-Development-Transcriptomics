...
count_de_genes <- function(de_tbl, cfg) {
  ...
  if (!all(c("log2FoldChange", "padj") %in% names(de_tbl))) {
    return(NA_integer_)
  }
...
safe_de_count <- function(de_tab, cfg) {
  ...
  if (!all(c("log2FoldChange", "padj") %in% colnames(de_tab))) {
    return(NA_integer_)
  }
...
count_de_union <- function(..., cfg) {
  ...
  hit_ids <- lapply(de_list, function(tbl) {
    if (!all(c("log2FoldChange", "padj") %in% names(tbl))) {
      return(character(0))
    }
    mask <- !is.na(tbl$padj) &
...
dataset_summary <- data.frame(
  Characteristic = c(
    "Total genes", "Developmental stages", "Biological replicates",
    "Platform", "Species", "GEO accession"
  ),
  Value = c(
    "16,489", "3 (P1, P12, P28)", "9 (3 per stage)",
...
if (length(de_list) == 0L) {
  ...
  plot_df <- dplyr::bind_rows(
    lapply(names(de_list), function(lbl) {
      df <- de_list[[lbl]]
      if (is.null(df) || !"log2FoldChange" %in% names(df)) {
        return(NULL)
      }
      data.frame(
        contrast       = lbl,
...
```{r volcano-plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Volcano Plot: P12 vs P1"}
library(ggplot2)
library(dplyr)
...
```{r hallmark-enrichment, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Hallmark Enrichment (P28 vs P1)"}
...
```{r pathway-velocity, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Pathway Activity Dynamics"}
if (exists("gse") && requireNamespace("msigdbr", quietly = TRUE)) {
  # Define key pathways to track
  pathways_of_interest <- c(
    "HALLMARK_G2M_CHECKPOINT", "HALLMARK_MYOGENESIS",
