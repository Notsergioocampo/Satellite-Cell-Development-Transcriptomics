...
if (is.null(de_p12_p1)) {
  ...
  text(0.5, 0.5,
       paste0(
         "DE results loaded but missing columns.\nColumns present:\n",
         paste(names(de_p12_p1), collapse = ", ")
       ),
       cex = 0.9)
} else {
...
if (is.null(hallmark_p28_p1)) {
  ...
  plot.new()
  text(0.5, 0.5,
       "No enrichment results available yet.\nRun the enrichment pipeline to generate hallmark_p28_p1.",
       cex = 1.1)
  ...
  df_clean <- df |>
    dplyr::filter(
      !is.na(padj),
      padj > 0,
      padj <= cfg_analysis$padj_threshold
    ) |>
    dplyr::arrange(padj) |>
    dplyr::slice_head(n = 15) |>
    dplyr::mutate(
      Pathway = factor(Description, levels = rev(Description))
    )
  ...
  if (nrow(df_clean) == 0) {
    ...
    plot.new()
    text(0.5, 0.5,
         "Enrichment results loaded\nbut no pathways pass the padj threshold.\nTry a higher padj_threshold in config.yaml.",
         cex = 1.1)
