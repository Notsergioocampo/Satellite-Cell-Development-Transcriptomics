...
# 1. Choose which matrix to use for UMAP
#    Adjust this line to match what you actually have:
#    - vst_mat
#    - rlog_mat
#    - norm_counts
#    etc.
...
} else if (exists("rlog_mat")) {
  expr_mat <- as.matrix(rlog_mat)
} else {
  expr_mat <- NULL
}

# 2. Compute UMAP if possible
if (!is.null(expr_mat)) {
  if (!requireNamespace("uwot", quietly = TRUE)) {
    stop("Package 'uwot' is required for UMAP but is not installed.")
  }

  # UMAP works on rows = samples, so transpose if rows = genes
  if (nrow(expr_mat) > ncol(expr_mat)) {
    expr_for_umap <- t(expr_mat)
  } else {
    expr_for_umap <- expr_mat
  }
  ...
  UMAP_results <- as.data.frame(umap_embed)
  colnames(UMAP_results) <- c("UMAP1", "UMAP2")

  # If you have a sample metadata table (e.g. coldata, sample_metadata),
  # merge it here. Adjust the name if needed.
  if (exists("sample_metadata")) {
    UMAP_results$sample_id <- rownames(expr_for_umap)
    UMAP_results <- dplyr::left_join(
      UMAP_results,
      sample_metadata,
